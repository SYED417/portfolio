  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (authError) {
      if (authError.message.includes("Invalid API key")) {
        setError(
          "Supabase API key is invalid. Update VITE_SUPABASE_ANON_KEY in your .env file with the correct anon key from Supabase Dashboard → Settings → API (it starts with eyJ...).",
        );
      } else {
    ...
    <div className="min-h-screen bg-background">
      <main className="container mx-auto px-4 pt-24 pb-16 flex items-center justify-center min-h-screen">
        <div className="w-full max-w-md">
          {message && (
            <div className="mb-4 p-3 rounded-lg bg-accent/10 border border-accent/20 text-sm text-foreground flex items-center gap-2">
          ...
          {showForgot ? (
              ...
              <CardContent>
                {resetSent ? (
                  ...
                  <form onSubmit={handleForgotPassword} className="space-y-4">
                    ...
                    <Button
                      type="submit"
                      className="w-full bg-gradient-aviation"
                      disabled={loading}
                    >
                      {loading ? (
            ...
            <Card className="bg-card border-border/50 shadow-aviation">
              <CardHeader className="text-center">
                ...
                <CardDescription className="text-muted-foreground">
                  Sign in to access your resume and exclusive content
                </CardDescription>
              </CardHeader>
              <CardContent>
